...
export default function GitDashboard() {
    ...
    const getStatusIcon = (status) => {
        switch(status) {
            case 'added': return <VscDiffAdded className="text-green-600" size={16} title="추가됨" />;
            case 'modified': return <VscDiffModified className="text-yellow-600" size={16} title="수정됨" />;
            case 'deleted': return <VscDiffRemoved className="text-red-600" size={16} title="삭제됨" />;
            case 'conflicted': return <VscWarning className="text-red-500" size={16} title="충돌됨" />;
            default: return null;
        }
    };

    useEffect(() => {
        if (workspaceId && activeProject) fetchBranchListApi(workspaceId, activeProject).then(setBranchList).catch(console.error);
        else setBranchList([]);
    ...
    const loadGitStatus = async () => {
        ...
        try {
            ...
            if (activeView === 'status') {
                ...
                const historyData = await fetchGitHistoryApi(workspaceId, activeProject, activeBranch || 'master');
                setHistoryLog(historyData || []);
            }
        } catch (error) { console.error("Git 상태 로드 실패:", error); } 
    ...
    const handleAbortMerge = async () => {
        ...
        try {
            ...
            setCommitMessage("");
            await loadGitStatus();
        } catch (error) { alert("병합 취소 실패: " + error.message); } 
        finally { setIsLoading(false); }
    };
    ...
    const handleCommit = async () => {
        ...
        try { 
            setIsLoading(true); 
            await commitChangesApi(workspaceId, activeProject, activeBranch || 'master', commitMessage, authorName, authorEmail); 
            alert("✅ 성공적으로 커밋되었습니다!"); 
            setCommitMessage(""); 
            await loadGitStatus(); 
            return true; 
        } 
        catch (error) { alert("커밋 실패: " + error.message); return false; } 
        finally { setIsLoading(false); }
    };
    ...
    const handleLinkGitUrlAndProceed = async () => {
        ...
        try {
            ...
            dispatch(updateProjectGitInfo({ projectName: activeProject, gitUrl: inputGitUrl }));
            alert("✅ Git 저장소가 성공적으로 연동되었습니다! 이어서 토큰 인증을 진행합니다.");
    ...
    const executeRemoteAction = async () => {
        ...
        try {
            setIsLoading(true);
            if (modalAction === 'push') { await pushToRemoteApi(workspaceId, activeProject, activeBranch || 'master', githubToken); alert("🚀 성공적으로 GitHub에 Push 되었습니다!"); } 
            else if (modalAction === 'pull') { await pullFromRemoteApi(workspaceId, activeProject, activeBranch || 'master', githubToken); alert("📥 성공적으로 GitHub에서 Pull 되었습니다!"); await loadGitStatus(); }
            setGithubToken(""); 
        } catch (error) { alert(error.message); setShowTokenModal(true); } 
        finally { setIsLoading(false); }
    };
    ...
    const handleContextMenuAction = async (action) => {
        ...
        try {
            setIsLoading(true);
            if (action === 'checkout') {
                if(window.confirm(`⚠️ 과거 커밋(${targetHash})으로 Checkout 하시겠습니까?\n커밋하지 않은 변경사항은 유실될 수 있으며, Detached HEAD 상태가 됩니다.`)) {
                    await checkoutCommitApi(workspaceId, activeProject, activeBranch || 'master', targetHash);
                    alert(`✅ HEAD가 ${targetHash} 커밋으로 이동했습니다!`);
                }
            } 
            else if (action === 'merge') {
                if(window.confirm(`현재 브랜치(${activeBranch || 'master'})에 이 커밋(${targetHash})을 병합하시겠습니까?`)){
                    await mergeCommitApi(workspaceId, activeProject, activeBranch || 'master', targetHash);
                    alert("✅ 병합 완료!");
                }
            } 
            else if (action === 'reset') {
                if(window.confirm(`⚠️ 경고: 현재 브랜치를 이 커밋(${targetHash}) 상태로 완전히 되돌리시겠습니까? 저장되지 않은 작업은 날아갑니다.`)){
                    await resetCommitApi(workspaceId, activeProject, activeBranch || 'master', targetHash);
                    alert("✅ 리셋 완료!");
                }
            }
            await loadGitStatus();
    ...
    const renderRefs = (refsStr) => {
        ...
        return refsStr.split(",").map(r => r.trim()).map((ref, idx) => {
            const isHead = ref.includes("HEAD"); const isRemote = ref.includes("origin/");
            ...
            else if (ref === 'master' || ref === 'main') bgColor = "bg-blue-50 text-blue-700 border-blue-200";
            return <span key={idx} className={`text-[10px] px-1.5 py-0.5 rounded-sm border font-semibold ${bgColor} mr-1.5 shrink-0 shadow-sm`}>{ref}</span>;
        });
    };

    if (!activeProject || !projectList || projectList.length === 0) return <div className="flex-1 flex items-center justify-center bg-gray-50 text-gray-400 flex-col gap-4"><VscSourceControl size={48} className="opacity-50" /><p>좌측 메뉴에서 프로젝트를 먼저 생성하거나 선택해주세요.</p></div>;
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-[#fefefe] relative">
                <div className="h-14 border-b border-gray-200 bg-white flex items-center justify-between px-6 shrink-0">
                    <div className="flex items-center gap-4">
                        <span className="font-bold text-lg">{activeView === 'status' ? 'File Status' : 'Commit History'}</span>
                        <div className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full border border-gray-200">
                            ...
                            <span className="text-xs text-blue-600 font-mono font-semibold">{activeBranch || 'master'}</span>
                        </div>
                        
                        <button 
                            onClick={async () => {
                                if (isMerging) return alert("⚠️ 병합 충돌 해결 중입니다. 타임머신을 탈 수 없습니다.");
                                try {
                                    setIsLoading(true);
                                    const target = activeBranch || 'master'; 
                                    await checkoutCommitApi(workspaceId, activeProject, activeBranch || 'master', target);
                                    alert(`✅ 원래 브랜치(${target})의 최신 상태로 복귀했습니다!`);
                                    await loadGitStatus();
                        ...
                        <VscRefresh className="cursor-pointer text-gray-400 hover:text-blue-600 transition-colors" title="새로고침" onClick={loadGitStatus}/>
                    </div>
                    <div className="flex items-center gap-3">
                        <button onClick={() => handleRemoteActionClick('pull')} className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50 shadow-sm transition-colors"><VscCloudDownload size={16} className="text-blue-600" /> Pull</button>
                        <button onClick={() => handleRemoteActionClick('push')} className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50 shadow-sm transition-colors"><VscCloudUpload size={16} className="text-green-600" /> Push</button>
                    </div>
                </div>
                    ...
                    {activeView === 'status' && isMerging && (
                        <div className="bg-red-50 border border-red-200 p-4 rounded-lg flex items-start justify-between shadow-sm">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-red-700 text-sm mb-1">병합 충돌(Merge Conflict)이 발생했습니다!</h3>
                                    <p className="text-xs text-red-600 leading-relaxed">
                                        아래 <b>'Conflicted Files'</b> 목록에 있는 파일들을 좌측 탐색기에서 열어 <code>&lt;&lt;&lt;&lt;&lt;&lt;</code> 와 <code>&gt;&gt;&gt;&gt;&gt;&gt;</code> 로 표시된 충돌 영역을 직접 수정하세요.<br/>
                                        수정이 완료되면 해당 파일의 <b>Resolve & Stage</b> 버튼을 누르고 커밋하여 병합을 완료할 수 있습니다.
                                    </p>
                                </div>
                            </div>
                            <button onClick={handleAbortMerge} className="bg-white border border-red-200 text-red-600 hover:bg-red-100 text-xs font-bold px-4 py-2 rounded shadow-sm transition-colors flex items-center gap-1 shrink-0">
                    ...
                    {activeView === 'history' ? (
                                        ...
                                        >
                                            ...
                                            {isCommit && (
                                                ...
                                                <div className="flex-1 flex items-center gap-2 truncate pr-4 h-full border-b border-gray-100/70 group-hover:border-transparent">
                                                    {renderRefs(log.refs)}
                                                    <span className="font-semibold text-gray-800 truncate" title={log.message}>{log.message}</span>
                                                </div>
                                                <div className="w-24 shrink-0 font-mono text-blue-600 font-medium text-center h-full flex items-center justify-center border-b border-gray-100/70 group-hover:border-transparent">{log.hash.substring(0, 7)}</div>
                                                <div className="w-28 shrink-0 text-gray-600 truncate text-center h-full flex items-center justify-center border-b border-gray-100/70 group-hover:border-transparent">{log.author}</div>
                                                <div className="w-32 shrink-0 text-right text-gray-500 text-xs h-full flex items-center justify-end border-b border-gray-100/70 group-hover:border-transparent">{log.date}</div>
                                                </>
                                            )}
                                        </div>
                                    )})
                                ) : (
                                    <div className="p-8 text-center text-gray-400 text-sm">커밋 기록이 없습니다.</div>
                                )}
                            </div>
                        </div>
                    ) : (
                        ...
                        {conflictedFiles.length > 0 && (
                            <div className="border border-red-300 rounded bg-red-50 shadow-sm overflow-hidden flex flex-col shrink-0 mb-2 animate-pulse-border">
                                <div className="bg-red-100 px-4 py-2 border-b border-red-200 flex justify-between items-center">
                                    <span className="text-sm font-bold text-red-800 flex items-center gap-2"><VscWarning/> Conflicted Files ({conflictedFiles.length})</span>
                                </div>
                                <div className="max-h-40 overflow-y-auto p-2 bg-white">
                                    {conflictedFiles.map((f, i) => (
                                        <div key={i} className="flex items-center justify-between px-2 py-1.5 hover:bg-red-50 rounded cursor-pointer group text-[13px]">
                                            <div className="flex items-center gap-2 truncate pr-2">
                                                {getStatusIcon(f.status)}
                                                <span className="font-mono text-red-700 font-semibold truncate">{f.path}</span>
                                            </div>
                                            <button onClick={() => handleStage(f.path)} className="opacity-0 group-hover:opacity-100 shrink-0 bg-white border border-red-300 text-red-600 px-2 py-0.5 rounded text-xs hover:bg-red-50 flex items-center gap-1 shadow-sm font-bold">
                        ...
                        <div className="flex gap-6 min-h-[250px]">
                            <div className="flex-1 flex flex-col border border-gray-200 rounded bg-white shadow-sm overflow-hidden">
                                <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 flex justify-between items-center"><span className="text-sm font-bold text-gray-700">Unstaged Files ({unstagedFiles.length})</span><button onClick={() => handleStage(".")} disabled={unstagedFiles.length === 0} className="text-xs text-blue-600 hover:underline disabled:text-gray-400 disabled:no-underline font-semibold">Stage All</button></div>
                                <div className="flex-1 overflow-y-auto p-2">{unstagedFiles.length > 0 ? (unstagedFiles.map((f, i) => (<div key={i} className="flex items-center justify-between px-2 py-1.5 hover:bg-blue-50 rounded cursor-pointer group text-[13px]"><div className="flex items-center gap-2 truncate pr-2">{getStatusIcon(f.status)}<span className="font-mono text-gray-700 truncate">{f.path}</span></div><button onClick={() => handleStage(f.path)} className="opacity-0 group-hover:opacity-100 shrink-0 bg-white border border-gray-300 px-2 py-0.5 rounded text-xs hover:bg-gray-100 flex items-center gap-1 shadow-sm"><VscArrowUp size={12}/> Stage</button></div>))) : (<div className="text-xs text-gray-400 flex items-center justify-center h-full">변경/추가된 파일이 없습니다.</div>)}</div>
                            </div>
                            <div className="flex-1 flex flex-col border border-gray-200 rounded bg-white shadow-sm overflow-hidden">
                                <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 flex justify-between items-center"><span className="text-sm font-bold text-gray-700">Staged Files ({stagedFiles.length})</span><button onClick={() => handleUnstage(".")} disabled={stagedFiles.length === 0} className="text-xs text-red-600 hover:underline disabled:text-gray-400 disabled:no-underline font-semibold">Unstage All</button></div>
                                <div className="flex-1 overflow-y-auto p-2">{stagedFiles.length > 0 ? (stagedFiles.map((f, i) => (<div key={i} className="flex items-center justify-between px-2 py-1.5 hover:bg-gray-100 rounded cursor-pointer group text-[13px]"><div className="flex items-center gap-2 truncate pr-2">{getStatusIcon(f.status)}<span className="font-mono text-gray-700 truncate">{f.path}</span></div><button onClick={() => handleUnstage(f.path)} className="opacity-0 group-hover:opacity-100 shrink-0 bg-white border border-gray-300 px-2 py-0.5 rounded text-xs hover:bg-gray-100 flex items-center gap-1 shadow-sm"><VscArrowDown size={12}/> Unstage</button></div>))) : (<div className="text-xs text-gray-400 flex items-center justify-center h-full">커밋할 파일이 없습니다.</div>)}</div>
                            </div>
                        </div>

                        <div className="border border-gray-200 rounded bg-white shadow-sm overflow-hidden flex flex-col shrink-0 mt-2">
                            <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
                                <span className="text-sm font-bold text-gray-700">{isMerging ? "Merge Commit Message" : "Commit Message"}</span>
                            </div>
                            <div className="p-4 flex gap-4">
                                <textarea className="flex-1 h-24 border border-gray-300 rounded p-3 text-sm resize-none outline-none focus:border-blue-500 font-mono" placeholder="커밋 메시지를 입력하세요 [Ctrl+Enter]" value={commitMessage} onChange={(e) => setCommitMessage(e.target.value)} onKeyDown={(e) => { if(e.ctrlKey && e.key === 'Enter') handleCommit(); }} disabled={isLoading}/>
                                <div className="w-48 flex flex-col gap-2">
                                    <button onClick={handleCommit} disabled={!commitMessage.trim() || stagedFiles.length === 0 || conflictedFiles.length > 0 || isLoading} className={`flex-1 text-white font-bold rounded flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm ${isMerging ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
                                        <VscCheck size={18} /> {isMerging ? "Merge 완료 커밋" : "Commit"}
                                    </button>
                                    <button onClick={handleCommitAndPush} disabled={!commitMessage.trim() || stagedFiles.length === 0 || conflictedFiles.length > 0 || isLoading} className="h-10 bg-gray-800 hover:bg-black text-white text-xs font-bold rounded transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                                        Commit & Push
                                    </button>
                                </div>
                            </div>
