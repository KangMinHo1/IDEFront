...
import { fetchAiAssistApi } from '../utils/api';

export default function AgentPanel() {
  const dispatch = useDispatch();
  
  const { agentMessages, selectedText } = useSelector(state => state.ui);
  const { workspaceId, activeProject, activeBranch, activeFileId, fileContents } = useSelector(state => state.fileSystem);
  ...
  useEffect(() => {
      const handleFocusChat = () => { if (inputRef.current) inputRef.current.focus(); };
  ...
  const handleSend = async (customQuery = null) => {
      ...
      try {
          ...
          const response = await fetchAiAssistApi({
              ...
              currentCode: activeFileId && fileContents[activeFileId] ? fileContents[activeFileId] : ''
          });

          if (response.success) {
              dispatch(addAgentMessage({ role: 'ai', content: response.explanation }));
              
              if (response.suggestedCode && response.suggestedCode.trim() !== '' && activeFileId) {
              ...
              dispatch(addAgentMessage({ role: 'ai', content: "❌ 에러: " + response.explanation }));
          }
      } catch (error) {
          dispatch(addAgentMessage({ role: 'ai', content: "❌ 서버와 연결할 수 없습니다. 통신 실패." }));
      } finally {
          setIsLoading(false);
      }
  };
  ...
  return (
    <div className="h-full bg-white flex flex-col font-sans border-l border-gray-200">
      <div className="h-9 px-4 flex items-center justify-between text-[13px] font-bold text-gray-800 bg-[#f8f9fa] border-b border-gray-200">
        ...
        <button onClick={() => dispatch(clearAgentMessages())} className="text-gray-400 hover:text-red-500 transition-colors" title="채팅 내역 지우기">
            <VscTrash size={16} />
        </button>
      </div>
            ...
            <div className="space-y-4">
                {agentMessages.map((msg, idx) => (
                    <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-fade-in-up`}>
                        <div className={`max-w-[90%] p-3 rounded-lg text-[13px] shadow-sm leading-relaxed whitespace-pre-wrap
                            ${msg.role === 'ai' 
                ...
                {isLoading && (
                    <div className="flex items-start animate-fade-in">
                        <div className="max-w-[90%] p-3 rounded-lg bg-white border border-gray-200 rounded-tl-none shadow-sm flex items-center gap-2 text-blue-500 text-[13px]">
                            <VscLoading className="animate-spin" /> AI가 코드를 분석 중입니다...
                        </div>
                    </div>
                )}
        ...
        <div className="flex flex-col gap-2">
            {selectedText && selectedText.trim() !== '' && (
                <div className="text-[11px] text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-1.5 rounded flex items-center gap-1.5 truncate shadow-sm">
                    ...
                    <span className="truncate">선택된 타겟 코드 ({selectedText.split('\n').length}줄) 감지됨</span>
                </div>
            )}
            ...
            <div className="flex flex-col gap-2 bg-white border border-gray-300 rounded-lg focus-within:border-blue-500 px-3 py-2 transition-all shadow-inner">
                ...
                <div className="flex justify-between items-center mt-1">
                    <span className="text-[10px] text-gray-400 font-bold">에디터에서 Ctrl+L 을 누르면 커서가 이동합니다.</span>
                    <button 
                        disabled={isLoading || !input.trim()}
                        className={`cursor-pointer p-1.5 rounded-md transition-colors ${input.trim() ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400'}`} 
                        onClick={() => handleSend()}
                    >
                        <VscSend size={14} />
                    </button>
