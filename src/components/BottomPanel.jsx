...
import { setActiveBottomTab, toggleTerminal, writeToTerminal } from '../store/slices/uiSlice';
import { VscChromeClose, VscTrash, VscAdd } from "react-icons/vsc";
...
export default function BottomPanel() {
    ...
    const { activeBottomTab, terminalOutput, isRunning, isDebugMode } = useSelector(state => state.ui);
        ...
        <div className="h-full flex flex-col bg-white font-sans text-[#333]">
            ...
            <div className="flex items-center justify-between bg-white h-9 border-b border-gray-200 select-none px-4">
                ...
                <div className="flex gap-3 items-center">
                    <VscAdd className="text-gray-500 hover:text-black cursor-pointer w-4 h-4" title="New Terminal" />
                    <VscTrash className="text-gray-500 hover:text-black cursor-pointer w-4 h-4" onClick={() => setOutputContent('')} title="Clear Output" />
                    <VscChromeClose className="text-gray-500 hover:text-black cursor-pointer w-4 h-4" onClick={() => dispatch(toggleTerminal())} title="Close Panel" />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-hidden relative bg-white">
                ...
                {/* 1. OUTPUT VIEW */}
                <div className={`h-full flex flex-col p-2 ${activeBottomTab === 'output' ? 'flex' : 'hidden'}`}>
                    <div className="flex-1 overflow-auto whitespace-pre-wrap font-mono text-xs leading-relaxed text-[#333]">
                        {outputContent || <span className="text-gray-400 italic pl-1">Run code to see output...</span>}
                        <div ref={outputEndRef} />
                    </div>
                    {(isRunning || isDebugMode) && (
                        <div className="flex items-center border-t border-gray-100 pt-2 mt-1">
                            <span className="text-blue-600 mr-2 text-xs font-mono font-bold">❯</span>
                            <input 
                ...
                {/* 2. TERMINAL VIEW */}
                <div className={`h-full w-full ${activeBottomTab === 'terminal' ? 'block' : 'hidden'}`}>
                    <Terminal />
                </div>
